(function () {
    ...
    Xflow.registerOperator("xflow.jsSimplex", {
        ...
        console.time("dataflowtimeJSsecond");
            ...
            for (var x = 0; x < width; x++) {
                for (var y = 0; y < height; y++) {
                        r = simplex.noise2D(x / 32, y / 32, t/16) * 0.5 + 0.5;
                        g = simplex.noise3D(x / 16, y / 16, t/16) * 0.5 + 0.5;
                        b = simplex.noise3D(x / 8, y / 8, t/16) * 0.5 + 0.5;
                        data[(x + y * width) * 4 + 0] = r * 255;
                        data[(x + y * width) * 4 + 1] = (r + g) * 200;
                        data[(x + y * width) * 4 + 2] = (r + g + b) * 200;
